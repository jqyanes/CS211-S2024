...
void accessData (
    ...
    mem_addr_t tag = addr >> (s+b);  // same as fully associative
    /* ... */

    // Cache hit (tag in cache set line matches tag)
    ...
    if ((*cache)[index].valid) (*eviction_count)++;
    //replace cache line with requested data (matching tag)
    cache_set_line_t req_data;
    req_data.tag = tag;
    req_data.valid = true;
    (*cache)[index] = req_data;
    /* ... */
}
